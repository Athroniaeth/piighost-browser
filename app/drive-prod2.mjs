...
await page.waitForFunction(
  ...
  null, { timeout: 400000 });
console.log("engine ready in", ((Date.now() - t0) / 1000).toFixed(1), "s (no model)");
...
console.log("rules only:", (await page.locator("main ul li").allInnerTexts()).length, "entities");
...
console.log("model fetched from the Hub in", ((Date.now() - t1) / 1000).toFixed(1), "s");
await page.getByRole("button", { name: /^Anonymiser$/ }).click();
await page.waitForFunction(() => document.body.textContent?.includes("ms modèle"), null, { timeout: 180000 });
await page.waitForTimeout(400);
console.log("status:", await page.locator("main p.text-xs").first().innerText());
console.log("entities:", (await page.locator("main ul li").allInnerTexts()).length);
